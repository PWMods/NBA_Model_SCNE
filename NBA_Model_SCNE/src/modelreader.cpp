...
void CModelReader::loadVertices(Mesh& mesh)
{
	...
	// CRITICAL FIX: Characters use 3-component after GeomDef drops W
	// Only balls/props with R16G16B16A16_SNORM keep 4 components
	if (posBf->getFormat() == "R16G16B16A16_SNORM") {
		// Ball/prop - keep the component count from setMeshVtxs
		// mesh.vertexComponents is already set correctly
	}
	else {
		// Character - always 3 components after processing
	...
	if (USE_DEBUG_LOGS) {
		...
		printf("\n[CModelReader] Built 3D Mesh: \"%s\" | Points: %d | Tris: %d | Components: %d",
			mesh.name.c_str(),
			numVerts,
			mesh.triangles.size(),
...
void CModelReader::loadWeights(Mesh& mesh)
{
	...
	// Get num total elements - vertices are ALWAYS 3-component after flattening
