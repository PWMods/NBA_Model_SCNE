...
void Mesh::generateAABBs()
{
	...
	// Handle both 3-component and 4-component vertices
	int stride = (vertexComponents > 0) ? vertexComponents : 3;

	if (coords.size() % stride != 0)
	{
		printf("\n[Mesh] Warning: vertices.size() (%zu) not divisible by stride (%d)", coords.size(), stride);
		return;
	}
	...
	for (int i = 0; i < numCoords; i += stride)
	{
...
{
	int stride = (vertexComponents > 0) ? vertexComponents : 3;
	int numVerts = vertices.size() / stride; // Use correct stride
	int array_size = (normals.size() == vertices.size()) ? stride : 4;
...
void Mesh::alignPosition(const bool use_blender_scale, const int num_components)
{
	...
	for (int i = 0; i < size; i += num_components)
	{
		...
		if (use_blender_scale)  // NBA to Blender (Y-up → Z-up)
		{
			vertices[i] = x;
			vertices[i + 1] = -z;   // Z becomes -Y in Blender
			vertices[i + 2] = y;    // Y becomes Z in Blender
		}
		else  // Blender to NBA (Z-up → Y-up)
		{
			vertices[i] = x;
			vertices[i + 1] = z;    // Blender Z becomes NBA Y
			vertices[i + 2] = -y;   // Blender Y becomes NBA -Z
		}

		// If 4-component, leave W unchanged
	}
}
...
void Mesh::alignNormals(const bool use_blender_scale, const int num_components)
{
	...
	for (int i = 0; i < size; i += num_components)
	{
		...
		{
			// NBA to Blender: Y-up to Z-up
			normals[i] = x;
			normals[i + 1] = -z;  // Z becomes -Y in Blender
			normals[i + 2] = y;   // Y becomes Z in Blender
		}
		else
		{
			// Blender to NBA: Z-up to Y-up (inverse of above)
			normals[i] = x;
			normals[i + 1] = z;   // Y in Blender becomes Z in NBA
			normals[i + 2] = -y;  // Z in Blender becomes -Y in NBA
		}
...
void Mesh::removeWAxis(std::vector<float>& data)
{
	...
	for (int i = 0; i < numItems; i++)
	{
		...
		new_data[target] = data[index];
		new_data[target + 1] = data[index + 1];
		new_data[target + 2] = data[index + 2];
		// Skip W component (data[index + 3])
	}
...
void MeshCalc::EncodeOctahedralNormal(Vec4* Encoded, const Vec3& Normal)
{
	...
	// ✓ FIXED: Wrap for negative Z (lower hemisphere)
	if (nz < 0.0f) {
...
{
	int stride = (mesh.vertexComponents > 0) ? mesh.vertexComponents : 3;
	int numVerts = mesh.vertices.size() / stride;
